"""
Prometheus Operational Metrics Exporter for caprini-vte-risk-calculator.
"""
import time
...
class SystemMetricsCollector:
    ...
    def export_prometheus_text(self) -> str:
        ...
        return (
            ...
            f"system_tasks_total {system=\"caprini-vte-risk-calculator\"} {self.tasks_total}\n\n"
            f"# HELP alerts_triggered_total Total count of alerts by urgency tier\n"
            f"# TYPE alerts_triggered_total counter\n"
            f"alerts_triggered_total {system=\"caprini-vte-risk-calculator\",urgency=\"CRITICAL_STAT\"} {self.critical_alerts_total}\n"
            f"alerts_triggered_total {system=\"caprini-vte-risk-calculator\",urgency=\"ELEVATED_RISK\"} {self.elevated_alerts_total}\n"
            f"alerts_triggered_total {system=\"caprini-vte-risk-calculator\",urgency=\"ROUTINE\"} {self.routine_tasks_total}\n\n"
            f"# HELP phi_outbound_blocks_total Total PHI outbound guard blocks\n"
            f"# TYPE phi_outbound_blocks_total counter\n"
            f"phi_outbound_blocks_total {system=\"caprini-vte-risk-calculator\"} {self.phi_blocks_total}\n\n"
            f"# HELP audit_chain_blocks_total Total HMAC-SHA256 audit blocks signed\n"
            f"# TYPE audit_chain_blocks_total counter\n"
            f"audit_chain_blocks_total {system=\"caprini-vte-risk-calculator\"} {self.audit_blocks_total}\n\n"
            ...
            f"task_processing_duration_avg_seconds {system=\"caprini-vte-risk-calculator\"} {avg_latency:.4f}\n"
        )
